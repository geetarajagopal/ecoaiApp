...
def serve_layout():
    ...
    data_combined["Current_Rain"] = np.where(data_combined["rain"][0] != 0, 1, 0)
    data_combined["Current_Snow"] = np.where(data_combined["snow"][0] != 0, 1, 0)
    ...
    label
    data_factors = [[prediction_temp.iloc[12],float(plot_data['Forecast_Humidity'][-1][0]),plot_data['Forecast_Temp'][-1][0]]] 
