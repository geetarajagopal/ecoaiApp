    ...
    #    current_time = datetime.now()
        ...
        if humidity is not None and temperature is not None:
            ...
            try:
                ...
                csv_writer.writerow([datetime.now(),round(humidity,2),round(temperature,2),response['current']['temp'],response['current']['humidity'],getValues('temp',response['hourly']),getValues('humidity',response['hourly']),location_value,getValues('rain',response['hourly']),getValues('snow',response['hourly'])])
                print("appended")
                break
