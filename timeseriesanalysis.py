...
data.columns=['Date','Humidity','Temperature','Current_Temp','Current_Humidity','Forecast_Temp','Forecast_Humidity','Location']
...
label = df_multivariatedata.index[-1]+50
idx = pd.date_range(str(label),periods=50,freq='5min')
df_forecast = pd.DataFrame(data=prediction,index=idx, columns=['Humidity','Temperature'])
...
df_withforecast = pd.concat([df_multivariatedata,df_forecast]).set_index(idx)
#df_withforecast.iloc[len(df_multivariatedata)+3]

df_multivariatedata[-300:-200]
